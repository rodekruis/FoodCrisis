...
d3.json("data/province_data.json", function(ind_prov){
	d.Rapportage = ind_prov;
	d3.json("data/Phil_provinces.json", function (geo_prov) {
		d.Districts = geo_prov;
		console.log(d);
		...
		var generateCharts = function (d){
			...
			if (admlevel === 2) { 
				...
				//document.getElementsByClassName('community-title')[0].innerHTML = name_selection;
				$('#jquery-test')[0].innerHTML = name_selection;
			}
			...
			$('#metric_label')[0].innerHTML = metric_label;
...
*/			;
			...
			var zoom_out = function(dest_level) {
			...
			info = function(id) {	// don't define with 'var' so it's a global function
				...
				if (!meta_icon[metric]) {metric_icon = 'modules/dashboards/img/undefined.png';}
				else {metric_icon = 'modules/dashboards/img/' + meta_icon[metric];}
